...
# Dataset QC 3: Establish and consistently perform control analyses
...
Here is an example part of the parcel-average timeseries files made by 3dROIstats. The timecourse is centered around zero due to the detrending and normalizing.
```{r}
options(repr.plot.width=9, repr.plot.height=2);  # specify notebook plot size
...
fname <- paste0(in.path, "sub-", sub.ids[2], "_ses-wave1bas_task-Stroop_run1_np2_L.txt"); 
if (file.exists(fname)) {
  np.tbl <- read.delim(fname);    

  print(dim(np.tbl));  # 540 TRs in the run, 500 parcels in the hemisphere + 2 label columns
  print(np.tbl[1:10,3:5]);  # columns with parcel names, average BOLD at each TR in each row
  ...
  # or plot it
  plot(np.tbl$Mean_17Networks_LH_SomMotB_Cent_8, type='l', col='seagreen', lwd=2)
}
...
fname <- paste0(in.path, "sub-", sub.ids[2], "_ses-wave1bas_task-Stroop_acq-mb4AP_run-1_events.tsv")
if (file.exists(fname)) {
...
plot.means <- function(rid, pid) {   # rid <- 1; pid <- 116;  
  ...
  lines(x=0:n.TRs, y=apply(all.means, 2, mean, trim=do.trim), col='black', lwd=2);  # across-subjects means
  box();
}
