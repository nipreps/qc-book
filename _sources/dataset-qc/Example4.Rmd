...
```
If I wanted to compile this report for a dozen participants I could make a dozen copies of the .rnw template, put one subject ID into the `sub.id` variable of each, and then compile each one, but that is tedious and it'd be easy to make a typo. It is more efficient to use a script to handle the duplication and text edits, such as this:
```{r}
options(eval=FALSE)
...
# check if out.path exists, and make it if not.
...
```
This type of script can also be used to put a single subject ID into multiple templates, to write a script to compile a group of files, etc.
