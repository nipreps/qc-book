...
# Principle 1: Is a task repetitive, boring, and/or error-prone? Script it!
...
do.convert <- function(sub.id) {    # sub.id <- "demoSub1";
  ...
  if (file.exists(paste0(in.path, fname))) {   # file exists, so read it and convert.
    print(paste("loading file for", sub.id)); 
    ...
    if (found.error == FALSE) {   # all correct, so store values and write
      ...
      #   print(paste("success! wrote", out.fname, "to disk."));
      # } else { 
      #   print(paste("ERROR: didn't write", out.fname, "to disk!!!!")); 
      # }

      # show a bit of the output csv
      print(e.tbl[,c(2:4,6:8)]);
      print("", quote=FALSE);  # for spacing
    }
...
# easier to run a group of subjects with a loop:
# for (sid in 1:length(sub.ids)) { do.convert(sub.ids[sid]); }

# list.files(in.path);  # print the names of the files in the input directory
