...
Surface temporal mean images are more difficult to check for preprocessing errors, because a correctly-made gifti file can always be displayed as a brain. But gross errors can still be spotted; my suggestion is to check for **tiger stripes** along the top of the hemispheres due to the central sulcus (arrows). Pages 15-17 of [132017_fMRI_movementSummary_fmriprep.pdf](https://osf.io/z62s5/) show an array of surface temporal mean images.
<img src="example2files/good.jpg"> 
<img src="example2files/failure.jpg"> 
...
By contrast, this participant responded to a few trials, sometimes correctly, but mostly did not, and the RAs noted that the person was sleepy: we do not want to include this person's data in the analyses. It is important to be able to check for this (and other) strange response patterns. For example, if an analysis calculated performance as the percent of trials with a correct response out of all trials with any response, this person's performance will seem **quite good** (most trials with a response are correct); the high proportion of trials without a response needs to be considered as well.
...
We can plot the same slice at different times (frames), and calculate statistics from individual frames or across time.
```{r}
options(repr.plot.width=8, repr.plot.height=2);  # specify size in jupyter
layout(matrix(1:5, c(1,5)));  # have three images in one row
par(mar=c(0.1, 0.1, 0.1, 0.1));   # specify plot margins

do.k <- 50;  # which k slice to plot
# so all frames will be plotted with the same color scaling, calulate the range across all TRs
z.lim <- c(min(in.img[,,do.k,]), max(in.img[,,do.k,]));

# plot one frame
image(in.img[,,do.k,1], col=gray(0:64/64), zlim=z.lim, xlab="", ylab="", axes=FALSE, useRaster=TRUE);
text(x=0.2, y=0.95, labels="frame 1", col='white');  # add a label

# plot with a loop
show.frames <- 2:8;  # which frames to plot. Trivial here, but can make esoteric selections.
# show.frames <- round(seq(from=1, to=dim(in.img)[4], length.out=8));   # which frames to plot
for (i in 1:length(show.frames)) {   # i <- 1;
  ...
  text(x=0.2, y=0.95, labels=paste("frame", show.frames[i]), col='white');  # add a label
  text(x=0.2, y=0.1, labels=paste0("max=", round(max(in.img[,,do.k,show.frames[i]]),2)), col='white');  # add a statistic
}
